	...
	<?php if(isset($arr['data'])){ ?>
		<table class="table table-hover">
			<thead>
				...
				<div class="addlead">
					<a href="<?php echo esc_url(admin_url('admin.php?')) ?>page=lead&view=post&pageSum=<?php echo isset($nextpage) && $nextpage >0 ? esc_attr( $nextpage ) : 1 ?>" class="btn btn-info">Add</a>
				</div>
				<div >
					...
					<th>ACTION</th>
						...
						<tbody class="filter_result">
							...
							<tr id ="id-<?php echo esc_attr($row['id']) ?>">		
								...
								<td>
									<a href="<?php echo esc_url(admin_url('admin.php?')) ?>page=lead&view=edit&id=<?php echo esc_attr($row['id']) ?>&pageNum=<?php echo esc_attr($pageNum) ?>" data-id="<?php echo esc_attr($row['id']) ?>" class="btn btn-success p-2"><i class="fa-solid fa-pen-to-square"></i></a>
									<button type="submit" data-toggle="modal" data-target="#exampleModal" class="delete btn btn-danger p-2 gethref" data-id="<?php echo esc_attr($row['id']) ?>"  data-href="<?php echo esc_url(admin_url('admin.php?')) ?>page=lead&iddel=<?php echo esc_attr($row['id']) ?>&pageNum=<?php echo esc_attr($pageNum) ?>"><i class="fa-solid fa-trash-can"></i></button>
								</td>
							</tr>
						<?php } ?>
						</tbody>
		...
		<nav class="pagina" aria-label="Page navigation example">
			<ul class="pagination">     
				...
				<?php } 
					for ($i = $pre; $i <= $next; $i++) { ?>
						<?php if ($i == $pageNum) { ?>
							<li class="page-item"><a class="page-link bg-warning" data-Num="<?php echo esc_attr($i) ?>" href="<?php echo esc_url(admin_url('admin.php?')) ?>page=lead&pageNum=<?php echo esc_attr($i); ?>"><?php echo esc_attr($i); ?></a></li>
						<?php } else { ?>
